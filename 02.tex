...
実際のブラウザ画面をキャプチャした画像を画像編集ソフトで加工し，視覚化システムで利用される表現方法をそれぞれ適用する．
...
図\ref{fig:ver-sheet1}は，各検索結果ごとの公開日を参考に，劣化の程度を調整した紙のテクスチャを適用している．

図\ref{fig:ver-sheet2}は，図\ref{fig:ver-sheet1}に加えて，劣化した紙のテクスチャが背景になじむように全体に紙のテクスチャを適用している．

紙の劣化を参考にしているため，各情報の古さが分かりやすい．家や図書館などで古い本を見た経験がある人ならば，紙の時間経過による劣化は古さを連想しやすい．
...
文字が記録されている媒体として金属が適していないため，金属のテクスチャを使う場合は，文字のフォントにも工夫する必要がある．
...
モノの腐食を参考にした，図\ref{fig:ver-corrosion1}，図\ref{fig:ver-corrosion2}は，暗い茶色や赤茶色をベースにして鮮度ごとに色が変化するように視覚化している．
...
また，図\ref{fig:ver-ink2}は，文字の色が背景色の白に薄まるように変化を加えている．
...
インクの劣化を参考にした視覚かは，段階的な変化の表現に適していないと考える．
...
元の検索画面に対して微小な変更のみを加えているため，不自然な点が少ない．また，段階的な変化が認識しやすく，時間経過を簡単に見て取れる．
...
古い書物などは紙自体の経年劣化の他にシミなどによる虫食いで欠落が見られる．そういった劣化の仕方を参考に鮮度を視覚化した．
...
しかし，ある程度の文字が欠損していても何となく読めてしまう部分があり，\ref{subsec:ver-col-ink}の視覚化と同様，鮮度に応じた段階的な変化は感じられない．
...
そこで，粒度の違うドットフォントを用いて情報の鮮度を視覚化した．
